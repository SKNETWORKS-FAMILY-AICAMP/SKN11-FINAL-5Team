...
from config.env_config import retriever, embedding, vectorstore
from langchain_core.output_parsers import StrOutputParser
import sys, os
sys.path.append(os.getcwd())
from config.env_config import llm_openai, llm_gemini
...
def classify_topics(user_input: str) -> list:
    ...
    chain = classify_prompt | llm_openai | StrOutputParser()
...
def run_business_planning_with_rag(user_input: str, use_retriever: bool = True, persona: str = "common"):
    ...
    # 히스토리 불러오기
    from queries import get_recent_messages 
    conversation_id = 4
    ...
    qa_chain = RetrievalQA.from_chain_type(
        ...
        chain_type_kwargs={"prompt": prompt},  # 직접 prompt 사용
...
def query_agent(request: AgentQueryRequest = Body(...)):
    ...
    if "린캔버스" in user_question:
        from queries import get_template_by_title
    ...
    from queries import insert_message
