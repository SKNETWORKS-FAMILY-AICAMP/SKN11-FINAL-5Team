...
def get_llm_auto():
    ...
    # 우선순위 모델 결정 (매 5회마다 스위치)
    if llm_state["use_count"] >= 5:
        llm_state["current"] = "gemini" if llm_state["current"] == "openai" else "openai"
        llm_state["use_count"] = 0
    llm_state["use_count"] += 1

    # 모델 호출 시도
    try:
        if llm_state["current"] == "gemini":
            print("🔄 Gemini 시도 중...")
            return ChatGoogleGenerativeAI(
                model="models/gemini-2.0-flash",  # 사용 가능한 모델로 변경
        ...
        print(f"⚠️ {llm_state['current']} 모델 호출 실패 → 전환 시도 중... 에러: {e}")

        # 전환
        fallback = "openai" if llm_state["current"] == "gemini" else "gemini"
        llm_state["current"] = fallback
        llm_state["use_count"] = 0

        if fallback == "gemini":
            print("🔁 OpenAI → Gemini 전환")
            return ChatGoogleGenerativeAI(
                model="models/gemini-2.0-flash",  # 사용 가능 모델 확인 필요
                google_api_key=GOOGLE_API_KEY
            )
        else:
            print("🔁 Gemini → OpenAI 전환")
            return ChatOpenAI(
                model="gpt-3.5-turbo",
                api_key=OPENAI_API_KEY
            )
...
def check_vectorstore_status():
    ...
    try:
        ...
        # 샘플 문서 확인
        sample_docs = vectorstore.similarity_search("마케팅", k=3)
...
def create_smart_retriever(user_input: str, topics: list = None):
    ...
    if topics:
        try:
            ...
            # 필터링된 검색 테스트
            test_docs = topic_retriever.invoke(user_input)
            if test_docs:
    ...
    try:
        general_retriever = vectorstore.as_retriever(
            search_kwargs={
                "k": 8,  # 더 많은 문서 검색
                "fetch_k": 20  # 더 넓은 후보군에서 선택
            }
        )
        
        test_docs = general_retriever.invoke(user_input)
...
def build_agent_prompt(topics: list, user_input: str, persona: str,history: list[str] = []):
    ...
    persona_config = get_persona_by_type(topics[0]) if topics else {}
    special_config = get_specialized_config(topics[0]) if topics else {}
...
def debug_templates():
    """템플릿 데이터 확인용 디버깅 엔드포인트"""
    try:
        # 전체 템플릿 조회
        all_templates = db.get_templates_by_type("전체")
        birthday_templates = db.get_templates_by_type("생일/기념일")
...
def debug_templates_by_type(template_type: str):
    """특정 타입의 템플릿 확인"""
    try:
        templates = db.get_templates_by_type(template_type)
        return {
