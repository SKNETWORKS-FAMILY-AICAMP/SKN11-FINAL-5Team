"""
통합 4단계 마케팅 에이전트 메인 실행 파일 (v4.0)
기존 시스템 + 새로운 4단계 체계적 시스템 통합
"""

import logging
import time
from typing import Dict, Any, Optional
from fastapi import FastAPI, HTTPException
from fastapi.middleware.cors import CORSMiddleware
from pydantic import BaseModel, Field

# 공통 모듈 임포트
from shared_modules import (
    get_config,
    setup_logging,
    create_success_response,
    create_error_response,
    get_current_timestamp
)

from marketing_agent.core.marketing_manager import Enhanced4StageMarketingManager

# 로깅 설정
logger = setup_logging("marketing_agent_v4", log_file="logs/marketing_agent.log")

# 설정 로드
config = get_config()

# FastAPI 초기화
app = FastAPI(
    title="Marketing Agent v4.0",
    description="통합 4단계 마케팅 전문 컨설팅 에이전트",
    version="4.0.0"
)

app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
    allow_credentials=True,
    allow_methods=["*"],
    allow_headers=["*"],
)

class IntegratedMarketingAgent:
    """통합 마케팅 에이전트 - 기존 + 4단계 시스템"""

    def __init__(self):
        # 4단계 시스템 (필수)
        self.enhanced_4stage_manager = Enhanced4StageMarketingManager()

    def get_system_status(self) -> Dict[str, Any]:
        """시스템 상태 조회"""
        return self.enhanced_4stage_manager.get_agent_status()
           
    def get_conversation_status(self, conversation_id: int) -> Dict[str, Any]:
        """대화 상태 조회"""
        return self.enhanced_4stage_manager.get_conversation_status(conversation_id)
        

# 마케팅 에이전트 인스턴스
marketing_agent = IntegratedMarketingAgent()

# 요청 모델
class UserQuery(BaseModel):
    """사용자 쿼리 요청"""
    user_id: int = Field(..., description="사용자 ID")
    conversation_id: Optional[int] = Field(None, description="대화 ID (멀티턴)")
    message: str = Field(..., description="사용자 메시지")

    class Config:
        json_schema_extra = {
            "example": {
                "user_id": 123,
                "conversation_id": 456,
                "message": "마케팅 전략을 수립하고 싶어요."
            }
        }

# API 엔드포인트
@app.post("/agent/query")
async def process_user_query(request: UserQuery):
    """멀티턴 대화 기반 사용자 쿼리 처리"""
    try:
        start_time = time.time()
        logger.info(f"멀티턴 쿼리 처리 시작 - user_id: {request.user_id}, message: {request.message[:50]}...")

        # 마케팅 에이전트로 처리
        result = await marketing_agent.enhanced_4stage_manager.process_user_query(
                user_input=request.message,
                user_id=request.user_id,
                conversation_id=request.conversation_id,
            )

        # 처리 시간 추가
        result["processing_time"] = time.time() - start_time

        logger.info(f"멀티턴 쿼리 처리 완료 - 시간: {result['processing_time']:.2f}초")
        return result

    except Exception as e:
        logger.error(f"멀티턴 쿼리 처리 중 오류 발생: {e}")
        return create_error_response(
            error_message=f"쿼리 처리 중 오류가 발생했습니다: {str(e)}",
            error_code="MULTITURN_QUERY_ERROR"
        )

@app.get("/agent/status")
def get_agent_status():
    """에이전트 상태 조회"""
    try:
        status = marketing_agent.get_system_status()
        return create_success_response(status)

    except Exception as e:
        logger.error(f"상태 조회 실패: {e}")
        return create_error_response(f"상태 조회 실패: {str(e)}", "STATUS_ERROR")

@app.get("/health")
def health_check():
    """상태 확인 엔드포인트"""
    try:
        health_data = {
            "service": "marketing_agent_v4",
            "status": "healthy",
            "timestamp": get_current_timestamp(),
            "version": "4.0.0",
            "features": [
                "4단계 마케팅 프로세스",
                "멀티턴 대화",
                "맞춤형 전략 수립",
                "목표 설정",
                "타겟 분석",
                "전략 기획",
                "실행 계획"
            ]
        }

        return create_success_response(health_data)

    except Exception as e:
        logger.error(f"헬스체크 실패: {e}")
        return create_error_response(f"헬스체크 실패: {str(e)}", "HEALTH_CHECK_ERROR")

# 메인 실행
if __name__ == "__main__":
    import uvicorn

    logger.info("=== Marketing Agent v4.0 시작 ===")
    logger.info("✅ FastAPI 기반 리팩토링 완료")
    logger.info("✅ 멀티턴 대화 시스템 적용")
    logger.info("✅ 4단계 마케팅 프로세스 적용")
    logger.info("✅ 공통 모듈 최대 활용")

    uvicorn.run(
        app,
        host=config.HOST,
        port=getattr(config, 'MARKETING_PORT', 8001),
        log_level=config.LOG_LEVEL.lower()
    )
